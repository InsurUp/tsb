...
const GuestRegistrationForm: React.FC<{ locale: string }> = ({ locale }) => {
  ...
  const handleParticipationTypeChange = (
    ...
    if (value === otherLabel) {
      window.open(
        "https://www.iucevents.com/international-insurance-summit",
        "_blank",
        "noopener,noreferrer"
      );
    }
  ...
  const validationSchema = z.object({
    ...
    email: z
      .string()
      .email(content.emailInvalid)
      .min(1, content.emailRequired),
    phone: z
      .string()
      .min(1, content.phoneRequired)
      .min(11, content.phoneLength)
      .max(11, content.phoneLength),
    ...
    kvkkConsent: z.boolean().refine((val) => val === true, {
      message: content.kvkkConsentRequired,
      path: ["kvkkConsent"],
    }),
  });
  ...
  const kvkkLinkText =
    locale === "tr" ? "KVKK Aydınlatma Metni" : "KVKK Privacy Notice";
    ...
    >
      <div className="max-w-[750px]! container">
        <div className="bg-white md:py-[70px] py-[30px] md:px-[50px] px-[20px] rounded-[30px] shadow-lg  mx-auto">
          <h2 className="sm:text-4xl text-3xl font-bold sm:mb-10 mb-5">
            {content.title}
          </h2>
          <form onSubmit={formik.handleSubmit}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 md:gap-[25px] gap-[20px] mb-[20px]">
              ...
              <div>
                ...
                {formik.touched.participationType &&
                formik.errors.participationType ? (
                  <div className="text-red-500 text-sm mt-1">
                    {formik.errors.participationType}
                  </div>
                ) : null}
