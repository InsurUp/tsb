...
const SponsorsModule: React.FC<{ locale: string }> = ({ locale }) => {
  const currentSponsorData = locale === 'tr' ? sponsorData.tr : sponsorData.en;

  return (
    <section className="md:py-[110px] sm:py-[70px] py-[30px]">
      <div className="container max-w-[600px]! mx-auto px-4">
        {currentSponsorData.map((categoryData, index) => (
          <div key={index} className="md:mb-10 mb-5 md:pb-10 pb-5 border-b border-solid border-gray-300 last:mb-0 last:pb-0 last:border-b-0">
            <h2 className=" text-4xl font-bold text-center mb-8">{categoryData.category[locale as keyof typeof categoryData.category]}</h2>
